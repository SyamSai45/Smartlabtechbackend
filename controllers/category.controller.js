...
// @desc    Get all categories with their products (including empty categories)
...
export const getCategoriesWithProducts = async (req, res) => {
  try {
    ...
    // Get ALL categories (not just active ones if you want)
    ...
    const categoriesWithProducts = await Promise.all(
      categories.map(async (category) => {
        // Find ALL products for this category (not just active)
...
// @desc    Get single category with its products (by ID or slug)
...
// @route   GET /api/categories/slug/:slug/with-products
export const getCategoryBySlugWithProducts = async (req, res) => {
  try {
    ...
    console.error('Error in getCategoryBySlugWithProducts:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// @desc    Get category by slug (without products)
